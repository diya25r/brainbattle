...
function QuizBattle() {
  ...
  return <section className="quiz-battle"><header className="quiz-battle__header"><div><p className="page-header__label">BATTLE</p><h1>{battle.subject}</h1><p>{battle.topic}</p></div><div className="quiz-battle__progress-label">Question {currentIndex + 1} of {questions.length}<small>YOU VS {opponent.name} · {opponentSubmitted ? 'SUBMITTED' : opponentConnected ? 'CONNECTED' : 'DISCONNECTED'}</small></div></header>
