function XPProgress({ level, xp }) {
  ...
  const remainingXp = Math.max(0, 500 - currentLevelXp)
  const progress = Math.min(100, (currentLevelXp / 500) * 100)

  return (
    <section className="xp-progress">
      <div className="xp-progress__heading"><div><p>XP PROGRESS</p><h2>LEVEL {level} <span>→</span> LEVEL {level + 1}</h2></div><strong>{currentLevelXp} / 500 XP</strong></div>
